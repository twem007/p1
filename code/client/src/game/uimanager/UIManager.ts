class UIManager {
	...
	// 	let stageWidth = AGame.R.app.stageWidth;
	// 	let stageHeight = AGame.R.app.stageHeight;
	// 	let widthPercentage = stageWidth / GameConst.CONTENT_WIDTH;
	// 	let hightPercentage = stageHeight / GameConst.CONTENT_HEIGHT;
	...
	// 	let stageHeight = AGame.R.app.stageHeight;
	// 	let widthPercentage = stageWidth / GameConst.CONTENT_WIDTH;
	// 	let height = 1 + (1 - stageWidth / GameConst.CONTENT_WIDTH);
	// 	obj.scaleX = widthPercentage * scale;
	// 	obj.scaleY = widthPercentage * scale;
	...
	// 	let stageHeight = AGame.R.app.stageHeight;
	// 	obj.x = pointX;
	// 	obj.y = stageHeight - (GameConst.CONTENT_HEIGHT - pointY);
	// }
	//  /**右下角适配
	//  * obj:要改变的对象
	//  * pointX:距离舞台的左边距离
	...
	// 	let stageHeight = AGame.R.app.stageHeight;
	// 	obj.y = stageHeight - (GameConst.CONTENT_HEIGHT - pointY);
	// 	obj.x = stageWidth-(GameConst.CONTENT_WIDTH- pointX);
	...
	// 	let stageWidth = AGame.R.app.stageWidth;
	// 	let stageHeight = AGame.R.app.stageHeight;
	// 	obj.x = stageWidth - (pointX * (stageWidth / GameConst.CONTENT_WIDTH));
	// 	obj.y = stageHeight - pointY;
	// }
	// /**高小于原来的比例调整对象缩放并修改Y值 */
	// public static changeHeightSize(obj: any, initSize: number, initY: number) {
	// 	let stageHeight = AGame.R.app.stageHeight;
	// 	let stagePercentage = stageHeight / GameConst.CONTENT_HEIGHT
	...
	// 	let stageWidth = AGame.R.app.stageWidth;
	// 	let stagePercentage = stageWidth / GameConst.CONTENT_WIDTH;
	// 	if (stagePercentage < 1) {
	...
	// 	let stageHeight = AGame.R.app.stageHeight;
	// 	if (stageHeight < GameConst.CONTENT_HEIGHT) {
	// 		let stagePercentage = GameConst.CONTENT_HEIGHT / stageHeight;
	// 		obj.scaleX = initSize * stagePercentage;
	// 		obj.scaleY = initSize * stagePercentage;
	// 	} else {
	// 		obj.scaleX = initSize;
	...
	// 	let stageHeight = AGame.R.app.stageHeight;
	// 	let stageHeightPercentage = stageHeight / GameConst.CONTENT_HEIGHT
	...
	// 	}
	// 	let stageWidth = AGame.R.app.stageWidth;
	// 	let stageWidthPercentage = stageWidth / GameConst.CONTENT_WIDTH
	...
	// /**读取配置表显示正确的弹窗操作 */
	// public static commonHint(id: number) {
	// 	let config: BroadcastConfig = CC.BroadcastConfig.getValBykey(id);
	// 	let text = config.promptContent;
	// 	if (config.promptType == 1) {
	// 		com_main.CommonHintView.m_pText = text;
	// 		com_main.CommonHintView.m_pCondition = CommonReturnType.BUTTON_OK;
	// 		AGame.R.notifyView(HintNav.COMMON_OPEN);
	// 	} else if (config.promptType == 2) {
	// 		AGame.R.app.hintLabel(text);
	// 	}
	// }
    // /**二级界面的出现效果 */
